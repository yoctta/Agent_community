"""Research-community extension tests.

Exercise the 15-agent research organisation: extended roles, groups,
directory, trust graph, token economy, server access, impersonation,
and security isolation.  Uses ``tests.stub_runtime.StubRuntime`` so
tests do not require an LLM API key.
"""
...
def engine(db, research_cfg):
    runtime = StubRuntime(rng=random.Random(1234))
    ...
    # Wire the services that tests exercise directly (metrics, defenses).
    eng.metrics_computer = MetricsComputer(
        db, csri_weights=research_cfg.experiment.csri_weights)
    eng.defense_manager = DefenseManager(
        research_cfg.defenses, db, eng.services, random.Random(1234))
    eng.init_world()
    return eng
...
def test_unknown_sender_is_flagged_untrusted_in_observation(engine):
    ...
    # it_victor is NOT in eng_julia's known_agents list. The observation
    # must surface a non-neighbor trust level so the LLM can decide
    # whether to act on the request (group_known, introduced, or unknown
    # are all acceptable — any value that is not ``trusted_neighbor``).
...
def test_research_smoke_run_produces_token_metric(db, research_cfg):
    runtime = StubRuntime(rng=random.Random(123))
    ...
    engine.defense_manager = DefenseManager(
        research_cfg.defenses, db, engine.services, random.Random(123))
    engine.init_world()
